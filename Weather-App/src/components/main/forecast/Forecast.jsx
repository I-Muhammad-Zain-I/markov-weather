...
const Forecast = (props) => {
  ...
  useEffect(() => {
    const getWeatherForecastDataHandler = async () => {
      const response = await getWeatherForecastData(
        props.coords.lat,
        props.coords.lon,
        unit
      );
  ...
  const labels = (paramList[parameter]?.[day] || []).map(
    (value) => `${value} ${paramUnits[parameter]}`
  );

  const images = (forecastData?.[day] || []).map((day) =>
    secondaryFindUrl(day.id, day.icon)
  );
  ...
  return (
    <>
      <h1 className="weather-forecast noto-h4-20" id="forecast">
        Weather Forecast
      </h1>
      <div className="forecast">
        <div className="forecast__dropdown-wrapper">
          <DayDropdown
            day={day}
            setDayHandler={setDay}
            optionList={forecastData.map((day) => day[0].date)}
          />
          <ParameterDropdown param={parameter} setParamHandler={setParameter} />
        </div>
        <div id="wrapper" className="chart-wrapper">
          <Line
            ...
            type="line"
            options={options}
            plugins={[xScaleImage, xScalePadding]}
          />
        </div>
      </div>
    </>
